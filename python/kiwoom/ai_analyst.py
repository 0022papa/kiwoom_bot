...
def ask_ai_to_buy(image_buffer, condition_id="0"):
    ...
    try:
        ...
        prompts = {
            "0": """
            당신은 '오전 급등주(Morning Breakout)' 전문가입니다. 1분봉 차트를 보고 판단하세요.
            [매수 기준]
            다음 세 가지 패턴 중 하나에 해당하면 매수로 판단하세요.
            패턴 1 (골든크로스 직전 수렴):
            1. 5 이동평균선이 20 이동평균선을 골든크로스하기 직전으로 수렴하고 있는가?
            2. 해당 수렴 구간에서 양봉이 발생하며 상승하려 하는가?
            3. 5 이동평균선이 상승 추세인가?
            패턴 2 (갭상승 돌파):
            1. 시초가에 갭상승(Gap-Up)이 발생했는가?
            2. 주가가 볼린저밴드 상단(또는 전고점)위에서 상승세를 유지하는가?
            3. 5 이동평균선, 20 이동평균선, 60이동평균선이 정배열이고 확산하려고 하는가?
            패턴 3 (상승추세 횡보 후 급등)
            1. 20 이동평균선의 기울기가 뚜렷한 우상향인가?
            2. 캔들이 5 이동평균선을 따라서 횡보하고 있는가?
            3. 최근 최저가가 5 이동평균선 또는 20 이동평균선에서 지지받고 있는가? (꼬리가 달리면 지지력 우수)
            """,
            "1": """
